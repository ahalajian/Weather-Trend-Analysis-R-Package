...
lowest_highest <- as.data.frame(lowest_highest)
colnames(lowest_highest) <- c("lowest", "highest")

for(i in 1:length(stations)){
  station <- stations[i]
  cycle <- yearly_cycle(station) #this gives us tibble of yearly cycle
  min <- min(cycle$expected_avg_temp, na.rm = TRUE)
  max <- max(cycle$expected_avg_temp, na.rm = TRUE)
  lowest_day <- cycle[which(cycle$expected_avg_temp == min), 1]
  highest_day <- cycle[which(cycle$expected_avg_temp == max), 1]
 
  lowest_highest$lowest[i] <- lowest_day
  lowest_highest$highest[i] <- highest_day

}

station_lh <- cbind(stations, lowest_highest)
station_lh

lowest_highest_combined <- cbind(interpolated_data, station_lh)
lowest_highest_combined

```
```{r}
quilt.plot(lowest_highest_combined$LONGITUDE, lowest_highest_combined$LATITUDE, lowest_highest_combined$lowest,
           xlab = "Longitude", ylab = "Latitude", main = "Coldest Day of the Year")

#make december 21 : day 1
# subtract 355
# add 365 to each day, subtract 355 then mod 365
quilt.plot(lowest_highest_combined$LONGITUDE, lowest_highest_combined$LATITUDE, lowest_highest_combined$highest,
           xlab = "Longitude", ylab = "Latitude", main = "Warmest Day of the Year")
```
